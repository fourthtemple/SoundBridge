...
JSON reports include a path-free `busProfile` for each probed plugin. Use it to call out coverage such as `effect-main`, `instrument-main`, `sidechain`, `multi-output`, and `multi-output-instrument`, plus flags for non-main-bus routing such as `sidechain-input`, `multi-input`, `multi-output`, inactive input/output buses, bus metadata at the report cap, nonsequential or duplicate bus indexes, active empty buses, and unusual bus types. The profile includes bounded bus/channel counts, active and inactive bus indexes, and compact anomaly counts so reports can describe unusual layouts without exposing plugin launch paths.
