...
JSON reports include a path-free `busProfile` for each probed plugin. Use it to call out coverage such as `effect-main`, `instrument-main`, `sidechain`, `multi-output`, and `multi-output-instrument`, plus flags for non-main-bus routing such as `sidechain-input`, `multi-input`, `multi-output`, inactive input/output buses, nonsequential or duplicate bus indexes, and unusual bus types. The profile includes bounded bus/channel counts plus active and inactive bus indexes so reports can describe unusual layouts without exposing plugin launch paths.

VST3 reports also include a path-free `vst3EventProfile` derived from bounded note-expression metadata. Use it to call out coverage such as `no-note-expressions`, `invalid-metadata`, `main-event-bus`, `non-main-event-bus`, `non-main-channel`, `text-expression`, `value-expression`, and multi-event-bus or multi-channel fixtures without including plugin paths, preset contents, or license data. Matrix entries include bounded note-expression counts, text/value/invalid counts, associated-parameter counts, type IDs, event-bus indexes, channels, and an explicit metadata-at-limit flag so reports can describe unusual event routing or capped partial SDK metadata without the full plugin metadata payload.

VST3 MIDI-controller event coverage is reported as `accepted`, `missing`, or `skipped-format`. An `accepted` result means the bounded control-change, pitch-bend, and channel-pressure probe batch was accepted by the worker path; it does not prove that a specific plugin exposes controller-to-parameter mapping targets. Matrix entries include path-free flags plus bounded controller family counts, controller event families, CC controller numbers, channels, and event-bus indexes used by the probe, including multi-family and non-main bus/channel breadcrumbs when exercised.

The summary report also counts VST3 program-list metadata as `listed`, `none`, `missing`, or `skipped-format`. VST3 program-data target coverage is reported as `targeted`, `unsupported`, `no-valid-programs`, `none`, `missing`, or `skipped-format`, with path-free flags such as `program-data-unsupported`, `empty-program-list`, `invalid-program-index`, `duplicate-program-list-id`, `duplicate-program-index`, `program-list-metadata-at-limit`, `program-metadata-at-limit`, and `bounded-target`. Matrix entries include bounded counts for unsupported, undisclosed, empty, missing-program, invalid-list, invalid-index, duplicate-list-id, duplicate-index, and valid candidate program-data targets, plus explicit metadata-at-limit booleans and VST3 program-data byte size when export succeeds; `0` bytes can be a valid empty SDK stream. Duplicate list IDs are ambiguous restore targets and should be treated as unsupported. Use this together with `vst3ProgramData` coverage to separate plugins that expose no SDK program lists, plugins whose program lists cannot produce bounded program data, plugins whose SDK program metadata reached report caps, and plugins whose program-data restore path failed.
