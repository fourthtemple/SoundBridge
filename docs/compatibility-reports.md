...
VST3 reports also include a path-free `vst3EventProfile` derived from bounded note-expression metadata. Use it to call out coverage such as `no-note-expressions`, `invalid-metadata`, `main-event-bus`, `non-main-event-bus`, `non-main-channel`, `text-expression`, `value-expression`, generated name fallbacks, and multi-event-bus or multi-channel fixtures without including plugin paths, preset contents, or license data. Matrix entries include bounded note-expression counts, text/value/invalid counts, invalid and defaulted route counts, associated-parameter counts, generated-name fallback counts, type IDs, event-bus indexes, channels, and an explicit metadata-at-limit flag so reports can describe unusual event routing or capped partial SDK metadata without the full plugin metadata payload.

VST3 MIDI-controller event coverage is reported as `accepted`, `missing`, or `skipped-format`. An `accepted` result means the bounded control-change, pitch-bend, and channel-pressure probe batch was accepted by the worker path; it does not prove that a specific plugin exposes controller-to-parameter mapping targets. Matrix entries include path-free flags plus bounded controller family counts, controller event families, CC controller numbers, channels, event-bus indexes, and defaulted or invalid number/route counts used by the probe, including multi-family and non-main bus/channel breadcrumbs when exercised. Parameter profiles separately count bounded VST3 MIDI-controller mapping breadcrumbs exposed by the SDK, including mapped parameter counts, assignment counts, duplicate assignments, controller numbers, event buses, and channels.

The summary report also counts VST3 program-list metadata as `listed`, `none`, `missing`, or `skipped-format`. VST3 program-data target coverage is reported as `targeted`, `unsupported`, `no-valid-programs`, `none`, `missing`, or `skipped-format`, with path-free flags such as `program-data-unsupported`, `empty-program-list`, `invalid-program-index`, `duplicate-program-list-id`, `duplicate-program-index`, `ambiguous-program-index`, `program-list-name-fallback`, `program-name-fallback`, `program-list-metadata-at-limit`, `program-metadata-at-limit`, and `bounded-target`. Matrix entries include bounded counts for unsupported, undisclosed, empty, missing-program, invalid-list, invalid-index, duplicate-list-id, duplicate-index, ambiguous duplicate-index groups, generated program-list/program-name fallbacks, and valid candidate program-data targets, plus explicit metadata-at-limit booleans and VST3 program-data byte size when export succeeds; `0` bytes can be a valid empty SDK stream. Duplicate list IDs and duplicate program indexes are ambiguous restore targets and should be treated as unsupported unless another unique bounded target exists. Use this together with `vst3ProgramData` coverage to separate plugins that expose no SDK program lists, plugins whose program lists cannot produce bounded program data, plugins whose SDK program metadata reached report caps or required generated names, and plugins whose program-data restore path failed.

Parameter metadata coverage is reported as `listed`, `none`, `missing`, or `at-limit`. An `at-limit` result means the daemon bounded the plugin's metadata at the configured parameter cap; include it in reports because it can explain incomplete generic-editor, automation, or program-change coverage. Parameter profiles add path-free counts for writable, automatable, read-only, generated-name fallback, display-value, unit, program-change, VST3 MIDI mapping, and VST3 unit metadata, including generated VST3 unit-name fallbacks and VST3 unit-to-program-list links when present, with categories such as `writable`, `automation-only`, `read-only`, `listed`, `none`, and `missing`.
