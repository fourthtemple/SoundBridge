...
`metadata` is optional bounded public class metadata for host caching and plugin browsers. It must not contain local filesystem paths. Current fields include `stableId`, `bundleIdentifier`, `version`, VST3 `vst3ClassId` / `vst3SdkVersion`, AudioComponent `componentType` / `componentSubType` / `componentManufacturer`, bounded AU `audioUnitHostProfile`, `lv2Uri`, `lv2BlockSizeProfile`, and path-free LV2 UI declaration fields (`lv2UiTypes`, `lv2UiCount`, `lv2UiBinaryCount`). VST3 plugin listings may use a short-lived brokered factory probe to refine public `name`, `vendor`, `category`, `kind`, and class identity values from SDK class metadata without exposing the bundle path to the browser. When available, `stableId` may be derived from the VST3 class id as `vst3:<vst3ClassId>`.
