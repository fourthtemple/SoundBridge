...
`renderEngine` is optional diagnostics. Current values include `bundle-worker`, `bundle-executable`, `native-example`, `native-au`, `native-vst3`, `native-lv2`, and `js-fallback`. JSON arrays remain the compatibility path and are still required for explicit multi-bus payloads. For low-latency main-bus effects processing, the reference client and daemon also support a binary WebSocket frame for `processAudioBlock`: an 8-byte `SBA1` header prefix, a bounded UTF-8 JSON request/response header, and planar little-endian Float32 channel data. The binary response carries the main `channels` buffer outside JSON and omits duplicated `outputBuses` sample arrays; hosts that need sidechain or multi-output bus audio should continue using the JSON bus payload until the bus-indexed binary frame is added. Production transports should keep moving toward dedicated browser-worker socket ownership and shared memory/ring buffers where available.
