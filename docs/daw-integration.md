...
This is the right policy for live sets: a slow, failed, or repeatedly over-budget effect should become a dry bypass, not silence and not an app crash. `processTimeoutMs` defaults to `0` so compatibility hosts can keep normal request-timeout behavior; live hosts can set a small bounded timeout from their block size and monitoring latency policy. `maxConsecutiveRenderBudgetMisses` defaults to `3`, can be set to `0` to observe without failing closed, and is capped to keep host policy bounded. `renderBudgetRecoveryBlocks` defaults to `0`; hosts can opt into automatic retry after that many dry blocks for transient CPU spikes, while processing errors and process timeouts still require `recreate()`. Binary frames, direct worklet-to-worker audio ports, `SharedArrayBuffer` rings where browser isolation allows them, atomic worker wakeups where supported, bounded shared-path in-flight requests, recycled worklet buffers plus transport-worker shared-input buffers, measured native render duration, per-block render-budget verdicts, measured response deadline lead/jitter, and bounded adaptive output latency reduce page-thread serialization work and smooth short jitter bursts. The worklet can also raise latency before dropouts when responses repeatedly arrive below its configured deadline-lead target. Production DJ use still needs deeper shared-memory tuning, latency calibration, and host scheduling policy.

## Current Prototype Limitations

- Main-bus and bus-indexed audio blocks use binary WebSocket frames in the reference web client. The demo opts into worker-owned WebSocket transport with a direct `AudioWorklet` audio port and `SharedArrayBuffer` rings when cross-origin isolation is available. The shared path wakes the transport worker with `Atomics.waitAsync`/`notify` where supported and reports whether the shared path is using atomic or timer wakeups. The worklet reports native render duration, render-budget misses, response deadline lead, and jitter in both render blocks and samples, so hosts can tell plugin-side render pressure from transport timing. The fallback port path recycles transferred input buffers after worker send, queues transferred binary output buffers without cloning them again, and reports bounded adaptive latency changes plus transport latency samples. JSON audio blocks remain the compatibility path for hosts that opt out of binary transport.
