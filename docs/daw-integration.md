...
- call `SoundBridgeAudioNode.setBypassed(true)` for Web Audio graph emergency fail-dry paths instead of destroying and recreating the plugin instance during a live set; the worklet outputs dry input and discards queued or pending wet responses from before the bypass transition
- read `SoundBridgeAudioNode.health` and listen for `latencychange`, `transport-pressure`, `render-budget-exceeded`, and `audio-error` when using the Web Audio graph path; health includes bypass state, render pressure, audio errors, transport latency, adaptive latency counters, deadline misses, stale output blocks, dropped input blocks, underruns, shared-audio drops, and the latest pressure/latency reasons
