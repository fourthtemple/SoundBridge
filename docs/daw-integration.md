...
- account for `getLatency()` in timeline scheduling and monitoring UI; `SoundBridgeLiveEffectRack.refreshLatency()`, `SoundBridgeAudioNode.refreshLatency()`, and render-response diagnostics keep plugin, transport, and total reported latency in host-readable sample and millisecond health
