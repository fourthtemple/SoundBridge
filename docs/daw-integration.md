...
This is the right policy for live sets: a failed or repeatedly over-budget effect should become a dry bypass, not silence and not an app crash. `maxConsecutiveRenderBudgetMisses` defaults to `3`, can be set to `0` to observe without failing closed, and is capped to keep host policy bounded. Binary frames, direct worklet-to-worker audio ports, `SharedArrayBuffer` rings where browser isolation allows them, atomic worker wakeups where supported, bounded shared-path in-flight requests, recycled worklet buffers plus transport-worker shared-input buffers, measured native render duration, per-block render-budget verdicts, measured response deadline lead/jitter, and bounded adaptive output latency reduce page-thread serialization work and smooth short jitter bursts. The worklet can also raise latency before dropouts when responses repeatedly arrive below its configured deadline-lead target. Production DJ use still needs deeper shared-memory tuning, latency calibration, and host scheduling policy.
