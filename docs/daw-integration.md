...
- read `SoundBridgeAudioNode.health` and listen for `latencychange`, `transport-pressure`, `transport-pressure-auto-bypassed`, `render-budget-exceeded`, `render-budget-auto-bypassed`, `audio-error-auto-bypassed`, and `audio-error` when using the Web Audio graph path; health includes bypass state, transport pressure, transport-pressure auto-bypass, render pressure, render-budget auto-bypass, audio errors, audio-error auto-bypass, plugin/transport/reported latency in samples and milliseconds, render-response plugin latency changes, adaptive latency counters, deadline misses, stale output blocks, dropped input blocks, underruns, shared-audio drops, and the latest pressure/latency reasons; `statsIntervalBlocks` is bounded and the live preset reports every 32 blocks by default
