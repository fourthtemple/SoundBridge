...
- host-owned serial rack chains and shared-frame batches expose bounded duration, process-budget misses, aggregate timeout pressure, repeated-miss trip status, health snapshots, budget/timeout/retry/recovery events, wet/dry transition fades, and optional dry-cooldown process-pressure recovery so DJ/live hosts can see when the combined rack or deck/send/master batch misses its block deadline even if each individual stage appears healthy
