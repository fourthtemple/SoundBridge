...
- binary WebSocket audio frames for main-bus and bus-indexed `processAudioBlock` buffers used by the web client and live effect rack, with copy-skipping typed-buffer normalization through browser packet encode, daemon packet decode, and native-worker command encoding
