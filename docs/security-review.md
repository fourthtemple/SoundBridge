...
| Regression coverage | Passing | `npm run smoke:security` exercises the fixes against a live daemon. Last recorded result: 116/116 checks passing. |
...
| Native worker IPC startup/command/line limits | Fixed | Native workers must report ready within a bounded timeout, worker commands must complete within bounded timeouts, native/example worker stdout response lines are capped before JSON parsing, and stderr diagnostic lines are capped before logging; missing ready handshakes, timed-out commands, oversized or unterminated responses, and oversized diagnostic lines reject pending commands and terminate the worker instead of stranding requests or growing daemon memory/logs. |
