...
| Regression coverage | Passing | `npm run smoke:security` exercises the fixes against a live daemon. Last recorded result: 164/164 checks passing. |
...
| LV2 bounded block-size options | Fixed | Compatible LV2 plugins that require `buf-size:boundedBlockLength` and `options#options` receive bounded instance options for minimum, maximum, nominal block length, and atom sequence size before instantiation; fixed and power-of-two block requirements remain unsupported until the render contract can honestly guarantee them. |
...
| Native worker file-grant operation boundary | Fixed | `useFileGrant` lets a paired host ask a compatible worker to consume an already attached grant for a known operation. The daemon validates instance ownership, grant ownership, instance attachment, operation-derived purpose/access/kind constraints, and worker support before resolving the absolute path, returns only path-free grant metadata to browsers, and sends the path only through bounded daemon-to-worker IPC. The reference VST3/AU/LV2 workers now implement bounded `restoreState` from granted worker-native state files and bounded `saveStateDirectory` writes into granted state directories; unsupported operations still fail closed. |
...
| Plugin editor/UI hosting | Native editor code exposes windowing, focus, clipboard, drag/drop, file-grant, and file-dialog surfaces. | Generic parameter editor sessions and the opt-in broker IPC contract are bounded today, including daemon-to-broker delivery of instance-attached grants. Remaining work is the platform-specific UI broker implementation for VST3/AU/LV2 editors; keep native UI code outside the daemon and broker UI actions explicitly. |
| Preset files, samples, caches, licensing | Plugins often expect filesystem and sometimes network access. | Keep `setPreset` limited to bounded listed parameter snapshots. The daemon now has an opt-in root-limited file grant foundation, a native approval broker contract, path-free browser responses, instance-scoped file grant attachments, native-editor-broker delivery of attached grants, a bounded native-worker `useFileGrant` operation boundary, and grant-backed state restore/save support in the reference VST3/AU/LV2 workers. Remaining work is non-state preset/sample/cache/license operations in each native worker and platform UI broker, while still avoiding ambient filesystem access and denying network access where the OS sandbox permits it. |
