...
| Bounded host transport context | Fixed | `processAudioBlock.transport` accepts bounded optional play state, tempo, time-signature, musical-position, cycle, and sample-position fields; the daemon rejects malformed values, VST3 workers re-validate before mapping to SDK `ProcessContext`, and AU workers re-validate before exposing CoreAudio host callbacks. |
