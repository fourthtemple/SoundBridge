...
| LV2 per-port bus routing | Fixed | The LV2 worker parses bounded indexed input-bus payloads, keeps bus 0 as the aggregate legacy bus, routes bus indexes `1..31` to matching parsed audio input ports, and returns bus-indexed output taps for parsed audio output ports. |
...
| Full plugin-hosting surface | Open roadmap | AU offline/splitter/format-converter profiles, remaining VST3 bus edge cases, LV2 port-groups and worker/UI support, plugin UI, and broader file access need feature-specific controls as they are implemented. |
...
| Advanced bus routing | Bad channel, block-size, sample-rate, sidechain, or multi-output negotiation can cause large allocations or crashes. | VST3 now uses explicit bounded bus buffers and bounded per-bus SDK speaker arrangement negotiation for active buses; AU routes active input elements and active output elements through bounded indexed buses; LV2 uses bounded aggregate bus 0 plus per-audio-port aux buses. Keep the same daemon and worker limits while expanding AU utility/offline profiles, LV2 port-groups semantics, and remaining format-specific bus edge cases. |
