...
| Regression coverage | Passing | `npm run smoke:security` exercises the fixes against a live daemon. Last recorded result: 113/113 checks passing. |
...
| Native worker IPC line limits | Fixed | Native/example worker stdout response lines are capped before JSON parsing; oversized or unterminated responses reject pending commands and terminate the worker instead of growing daemon memory. |
