...
| Regression coverage | Passing | `npm run smoke:security` exercises the fixes against a live daemon. Last recorded result: 150/150 checks passing. |
...
| Plugin editor/UI hosting | Native editor code exposes windowing, focus, clipboard, drag/drop, and file-dialog surfaces. | Generic parameter editor sessions and the opt-in broker IPC contract are bounded today. Remaining work is the platform-specific UI broker implementation for VST3/AU/LV2 editors; keep native UI code outside the daemon and broker UI actions explicitly. |
| Preset files, samples, caches, licensing | Plugins often expect filesystem and sometimes network access. | Keep `setPreset` limited to bounded listed parameter snapshots. The daemon now has an opt-in root-limited file grant foundation with path-free browser responses; remaining work is wiring those opaque grants into native workers and UI brokers for real preset/sample/cache/license file consumption, while still avoiding ambient filesystem access and denying network access where the OS sandbox permits it. |
