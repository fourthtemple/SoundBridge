...
| AU/LV2 main bus buffers | Fixed | Installed AU and LV2 workers now consume explicit bounded bus-0 input buffers and return bus-0 `outputBuses` that mirror `channels`, so hosts see the same main-bus response shape across VST3/AU/LV2 while advanced LV2 routing and AU multi-output remain roadmap work. |
| AU input-element routing | Fixed | The AU worker parses bounded indexed input-bus payloads, installs render callbacks for active CoreAudio input elements where setup succeeds, and routes callback `busNumber` to the matching explicit bus buffer while keeping output routing conservative. |
...
| Advanced bus routing | Bad channel, block-size, sample-rate, sidechain, or multi-output negotiation can cause large allocations or crashes. | VST3 now uses explicit bounded bus buffers and bounded per-bus SDK speaker arrangement negotiation for active buses; AU routes active input elements from bounded indexed bus buffers and returns conservative bus-0 output; LV2 uses explicit bounded main-bus input/output buffers. Keep the same daemon and worker limits while expanding AU multi-output, LV2 sidechains, LV2 multi-output routing, and remaining format-specific bus edge cases. |
