...
| Regression coverage | Passing | `npm run smoke:security` exercises the fixes against a live daemon. Last recorded result: 155/155 checks passing. |
...
| Preset files, samples, caches, licensing | Plugins often expect filesystem and sometimes network access. | Keep `setPreset` limited to bounded listed parameter snapshots. The daemon now has an opt-in root-limited file grant foundation, a native approval broker contract, path-free browser responses, and no browser-supplied path mode unless explicitly enabled for development; remaining work is passing opaque grant ids into native workers and UI brokers for real preset/sample/cache/license file consumption, while still avoiding ambient filesystem access and denying network access where the OS sandbox permits it. |
