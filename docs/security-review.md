...
| Regression coverage | Passing | `npm run smoke:security` exercises the fixes against a live daemon. Last recorded result: 184/184 checks passing. |
| Installed-plugin compatibility probe | Passing | `npm run probe:installed` starts a temporary paired loopback daemon with an explicit origin allowlist and bounded request sizes so real VST3/AU/LV2 create/listed-preset/VST3 program-data/parameter/state/file-grant/MIDI/render/layout and profile-aware automation checks can be repeated without weakening the production security model. The default probe format set is VST3/AU/LV2; unsupported `SOUNDBRIDGE_PROBE_FORMATS` values fail before daemon launch. `SOUNDBRIDGE_PROBE_REPORT=summary` prints feature-coverage counts, and `SOUNDBRIDGE_PROBE_NATIVE_EDITOR_BROKER=1` also checks native editor broker open/close behavior against those installed instances. |
