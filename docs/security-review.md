...
| Bounded host transport context | Fixed | `processAudioBlock.transport` accepts bounded optional play state, tempo, time-signature, musical-position, cycle, and sample-position fields; the daemon rejects malformed values and the VST3 worker re-validates before mapping them to SDK `ProcessContext`. |
