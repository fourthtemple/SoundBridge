...
| Regression coverage | Passing | `npm run smoke:security` exercises the fixes against a live daemon. Last recorded result: 121/121 checks passing. |
...
| Bounded VST3 unit/program metadata | Fixed | VST3 parameters expose capped SDK unit metadata where `IUnitInfo` provides it; program-change parameters are marked; associated and plugin-level program-list names are exposed only as capped display metadata, with selection still routed through validated `setParameter` calls. |
...
| Bounded VST3 note-expression values | Fixed | VST3 workers expose capped `INoteExpressionController` value metadata and accept bounded value-style note-expression events with explicit note ids; the daemon rejects malformed values and rejects note-expression events for non-VST3 workers. |
...
| Full plugin-hosting surface | Open roadmap | AU offline/splitter/format-converter profiles, remaining VST3 bus/program-data/text-expression edge cases, LV2 UI and advanced extension support, plugin UI, and broader file access need feature-specific controls as they are implemented. |
