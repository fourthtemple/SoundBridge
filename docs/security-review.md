...
| Regression coverage | Passing | `npm run smoke:security` exercises the fixes against a live daemon. Last recorded result: 117/117 checks passing. |
...
| Native worker IPC startup/command/line limits | Fixed | Native workers must report ready within a bounded timeout, worker commands must complete within bounded timeouts, native/example worker stdout response lines are capped before JSON parsing, stderr diagnostic lines are capped before logging, and cumulative stderr diagnostics are budgeted per worker; missing or invalid ready handshakes, timed-out commands, oversized, unterminated, or malformed responses, oversized diagnostic lines, and diagnostic floods reject pending commands and terminate the worker instead of stranding requests, desynchronizing IPC, or growing daemon memory/logs. |
...
| Full plugin-hosting surface | Open roadmap | AU offline/splitter/format-converter profiles, remaining VST3 bus edge cases, LV2 UI and advanced extension support, plugin UI, and broader file access need feature-specific controls as they are implemented. |
