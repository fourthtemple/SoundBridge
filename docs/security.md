...
- Treat worker process sandboxing as an explicit extended hardening profile, not as the default compatibility assumption.
...
| AU host profiles | The AU registry includes offline effects and system format-converter/splitter units that are not safe to treat as ordinary realtime main-bus effects. | Classify AU listings with bounded public `audioUnitHostProfile` metadata, expose supported realtime utility profiles explicitly (`realtime-format-converter`, `realtime-multi-source-merger`, `realtime-multi-output-splitter`), keep incompatible AU profiles discovery-only until each one has a dedicated bounded host profile, and reject `createInstance` before spawning workers for unsupported profiles. |
...
| Plugin editor/UI hosting | Native editor code can open windows, dialogs, clipboard, drag/drop, file grants, and platform UI surfaces. | Expose bounded per-plugin `editorKinds` for host UI gating; support bounded generic parameter editor sessions now; enable native editor windows only through an explicit separate UI broker process spawned without a shell, keep broker IPC bounded by worker limits, require daemon opt-in before broker file-dialog, clipboard, or drag/drop capabilities become browser-visible, send the effective UI capability policy to the broker, pass only instance-attached grants to the broker, and preserve web/local host ownership checks. |
| Preset files, samples, caches, licensing | Plugins may expect broad filesystem or network access. | Keep in-protocol presets to bounded listed parameter snapshots; broker user-visible file access for real files with opaque session-owned grants, reject paths outside configured roots and symlink escapes, use an explicit native approval broker for production grants, attach grants to session-owned plugin instances without exposing paths, expose bounded per-plugin `fileGrantOperations`, resolve attached grants only inside daemon-owned code, support bounded grant-backed worker-native state restore, preset-state load, and state-directory save, forward any remaining native-worker file operations only through bounded operation-specific IPC, keep browser-supplied paths behind development opt-in, and make any denial of ambient filesystem or network access part of an explicit sandboxed-worker profile. |
