...
| AU host profiles | The AU registry includes offline effects and system format-converter/splitter units that are not safe to treat as ordinary realtime main-bus effects. | Classify AU listings with bounded public `audioUnitHostProfile` metadata, keep incompatible AU profiles discovery-only until each one has a dedicated bounded host profile, and reject `createInstance` before spawning workers for units outside the current realtime main-bus profile. |
