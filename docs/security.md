...
| State save/restore | Opaque blobs can be huge or maliciously malformed. | VST3/AU now enforce blob-size limits, keep state opaque, bind it to the producing instance/session, and never interpret it as a path or command; basic LV2 control-port state is bounded and keyed only to known ports; LV2 extension state must follow the same rule before it is enabled. |
...
| LV2 extensions | Atom MIDI, extension state, worker, UI, and other extensions require host-provided feature data and callbacks. | Keep unsupported extensions disabled; enable each extension only with explicit feature structs, bounded data, ownership checks, and worker containment. |
