...
| AU | Active macOS `.component` scanner plus AudioComponent registry metadata | Active CoreAudio AudioComponent worker | macOS-only. Runs installed AU binaries in a separate worker process, exposes path-free AudioComponent identifiers, and maps bounded host transport context through CoreAudio host callbacks. |
