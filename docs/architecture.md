...
The `AudioWorkletProcessor` never blocks. It copies input blocks, posts them over an optional direct `MessagePort` to the web client's transport worker, and consumes returned processed blocks by `blockId` at a configured block latency so out-of-order WebSocket responses cannot reshuffle audio during live effects processing. The page thread sets up transport, while live blocks use `SharedArrayBuffer` input/output rings when cross-origin isolation is available and fall back to the direct `MessagePort` path otherwise. The shared path wakes the transport worker with `Atomics.waitAsync`/`notify` where supported, with a bounded timer fallback, and the worker stops draining the shared input ring when its browser-side audio requests reach the configured in-flight limit. The worklet recycles input buffers and shared-output copy buffers, and the transport worker recycles shared-input copy buffers after WebSocket send, so sustained live processing does not allocate per block after warmup. The fallback path recycles transferred input buffers after worker send and queues transferred binary output buffers without cloning them again. Both paths can adapt output latency within bounded min/max blocks when repeated misses show worker/daemon jitter, can preemptively raise latency when response deadline lead repeatedly falls below the configured target, and can raise latency when measured response jitter crosses the configured bounded threshold. Preemptive raises insert a controlled dry safety block to grow buffering instead of counting that block as an underrun. If the target block is missing or stale, the worklet falls back to dry audio and reports underrun, stale-output, dropped-input, shared-ring, shared wake-mode, input/output-buffer, current and peak queue-depth, latency-block, transport-latency samples, response deadline-lead, render-duration, render-budget verdict, and windowed response-jitter stats; the SDK health converts plugin, transport, and reported latency samples into bounded milliseconds for host monitoring.
...
- response deadline margin and jitter in render blocks/samples
