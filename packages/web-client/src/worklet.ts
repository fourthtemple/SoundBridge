export class SoundBridgeAudioProcessor extends AudioWorkletProcessor {
  ...
  private sharedInputDroppedBlocks = 0; private sharedOutputDroppedBlocks = 0; private sharedInputQueuedMaxBlocks = 0; private sharedOutputQueuedMaxBlocks = 0;
  ...
  private handleMessage(message: unknown): void {
    ...
    if (typed.type === "audio-error") {
      ...
      this.port.postMessage({ type: "audio-error", blockId: typed.blockId, error: typed.error, ...(sharedStatus ?? {}) });
      return;
    }
    ...
    const outputChannels = new Array<Float32Array>(Math.min(this.outputChannels, typed.channels.length));
    for (let channelIndex = 0; channelIndex < outputChannels.length; channelIndex += 1) {
      outputChannels[channelIndex] = this.outputChannelBlock(typed.channels[channelIndex]);
    }
  ...
  private copyInputBlock(input: Float32Array[], frames: number): Float32Array[] {
    const channels = new Array<Float32Array>(this.outputChannels);
  ...
  private queueSharedOutputBlock(
    ...
    const outputChannels = new Array<Float32Array>(channels);
  ...
  private reportedLatencySamples(transportLatencySamples = this.transportLatencySamples()): number { return Math.min(1_048_576, transportLatencySamples + this.pluginLatencySamples); }
