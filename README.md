...
- AU: installed macOS Audio Units through the CoreAudio host worker, including path-free public AudioComponent metadata, parameter metadata, generic parameter editor sessions, parameter writes, bounded automation event lists and per-block curves, conservative per-bus layout reporting, rendering, bounded note, CC, pitch-bend, pressure, and program-change events where supported, bounded latency/tail reporting, and opaque state save/restore.
