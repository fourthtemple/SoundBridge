...
For deck, send, or master chains, use `createLiveEffectRackChain()` with `chain.timing`, `createLiveEffectRackChainCalibrationWindow()`, and `scheduler.updateFromChainHealth(chain.health)` so the host can soundcheck and compensate one serial effects chain instead of every plugin slot individually.
