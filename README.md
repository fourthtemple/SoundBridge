...
- VST3: installed audio effects through the Steinberg VST3 SDK host worker, including path-free public scanner metadata, parameter metadata, bounded program-list metadata where plugins expose VST3 program-change parameters, parameter writes, bounded automation event lists, bounded note/poly-pressure events, VST3 MIDI-controller parameter mapping where plugins expose it, negotiated per-bus layout reporting, bounded bus-aware rendering for active VST3 buses, bounded latency/tail reporting, and opaque state save/restore.
