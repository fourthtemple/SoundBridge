...
Preset/sample/cache/license file access is also not ambient. The daemon exposes an opt-in file grant foundation that stays disabled unless `SOUNDBRIDGE_FILE_GRANT_ROOTS` names explicit local roots; browser responses receive opaque grant ids and display names, not absolute paths. Production-style approvals use an explicit native broker configured with `SOUNDBRIDGE_FILE_GRANT_BROKER_PATH`, then hosts can attach the opaque grant to a session-owned plugin instance. `useFileGrant restoreState` and `useFileGrant saveStateDirectory` are implemented for the reference VST3/AU/LV2 native workers so they can restore and save bounded worker-native state files without exposing local paths to browser code. Other operations such as `loadSample`, `loadPreset`, cache, and license handling remain operation-specific host-adapter work; the absolute path is resolved only inside the daemon and sent only over bounded worker IPC. Browser-supplied path strings additionally require `SOUNDBRIDGE_FILE_GRANT_ALLOW_BROWSER_PATHS=1` and are intended only for development and test harnesses.
