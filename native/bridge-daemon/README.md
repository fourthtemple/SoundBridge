...
- Complete deeper VST3 musical-control support where the SDK exposes a safe host path, including unit selection, note-expression features, and richer program-list behavior beyond bounded program-change parameter metadata, with event-count, byte-size, timing-offset, channel/note, and worker-queue limits.
- Expand VST3/AU/LV2 automation from bounded event lists to higher-density curves/interpolation only with parameter-count, string-length, event-count, timing-offset, and per-instance rate limits.
