...
- Complete deeper VST3 musical-control support where the SDK exposes a safe host path, including program/unit selection and note-expression features, with event-count, byte-size, timing-offset, channel/note, and worker-queue limits.
...
- Add LV2 file-backed state, worker, UI, and extension-feature support behind the same worker, validation, state, bus, file-broker, and sandbox rules. Basic LV2 control-port state, bounded portable POD state, and bounded atom MIDI delivery are already supported for compatible plugins.
