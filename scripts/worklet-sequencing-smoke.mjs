...
sharedTransportPort.onmessage({ data: { type: "audio-error", blockId: 12, error: "shared timeout", sharedTransportInFlightBlocks: 0 } });
assert(sharedProcessor.sharedTransportStats.sharedTransportInFlightBlocks === 0 && sharedMainPort.messages.some((message) => message.type === "audio-error" && message.blockId === 12 && message.sharedTransportInFlightBlocks === 0), "shared worklet transport forwards block-attributed worker error status");
