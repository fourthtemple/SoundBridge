...
function summarizeCompatibilityMatrix(results, options) {
  return results.map((result) => {
    ...
    return removeEmpty({
      ...
      parameterInvalidVst3MidiMappingRouteCount: safeMatrixInteger(
        result.parameterProfile?.invalidVst3MidiMappingRouteCount,
        0,
        256
      ),
      parameterInvalidVst3MidiMappingControllerCount: safeMatrixInteger(
        result.parameterProfile?.invalidVst3MidiMappingControllerCount,
        0,
        256
      ),
      ...
      parameterDisplayInput: safeMatrixText(parameterDisplayInputStatus(result), 64),
      ...
      hostTransport: safeMatrixText(hostTransportStatus(result), 64),
...
function summarizeFeatureCoverage(results, options) {
  return {
    ...
    parameterDisplayInput: countBy(results, parameterDisplayInputStatus),
